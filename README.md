...
| `skill-management` (`audit-skills-against-best-practices`) | developer-tools | Systematic bulk quality audit of all skills in a plugin workspace: dispatches parallel sub-agents per batch, evaluates each skill against a canonical checklist (skill-creator + skill-creator-extra-tips + 4 additional criteria), classifies findings by severity (CRITICAL/SHOULD_FIX/NICE_TO_HAVE), and auto-files actionable items via use-local-skills-issue-tracker |
| `skill-management` (`enrich-skill-via-research`) | developer-tools | Research external sources (official docs, team wikis, post-mortems, code search) to fill gaps in an existing skill's SKILL.md: failure modes, anti-patterns, troubleshooting steps, and operational gotchas. Distinct from skill-creator (creates from scratch) and audit-skills-against-best-practices (checks compliance) |
