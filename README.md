...
| `package-management` (`audit-global-tool-updates`, `configure-dependency-cooldown`) | developer-tools | Package-manager supply-chain hygiene: scan globally installed CLI tools for safe updates, and configure dependency cooldowns (npm, pnpm, Yarn, Bun, Deno, uv, pip, Poetry, Bundler, Cargo) to delay installing newly published versions |
...
| `software-management` (`audit-dependency-advisories`, `audit-software-upgrade`, `review-supply-chain-risk`) | developer-tools | Three skills: audit a candidate upgrade of a GitHub-hosted app before installing (audit-and-report only, never installs): resolve versions, review every commit for malware/tampering via review-supply-chain-risk, audit dependency changes via audit-dependency-advisories, evaluate trust model, emit SAFE/REVIEW/BLOCK verdict, save to `/tmp`; review any git diff, PR, or package tarball for malware/supply-chain tampering and critical/high vulnerabilities (SAFE/REVIEW/BLOCK); check dependencies against advisory databases (GitHub Advisory DB, OSV) and flag versions published less than 72 hours ago. Ships per-app specifications (template + cmux example) |
