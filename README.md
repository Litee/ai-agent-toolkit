...
| `communication` (`communicate-well`, `watch-communication-channels`) | productivity | Two skills: async communication guidelines (value test, message style, frequency, anti-patterns, channel-type rules); channel watcher framework with monitoring method selection hierarchy (defers to domain-specific watchers when available, falls back to team agent architecture, exponential backoff, 7-day timeout, session-resume restoration via SessionStart hook) |
