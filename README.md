...
/plugin install software-management@litee-claude-code-plugins
...
| `package-management` (`configure-dependency-cooldown`, `audit-global-tool-updates`) | developer-tools | Two skills: configure dependency cooldowns across all major package managers (npm, pnpm, Yarn, Bun, Deno, uv, pip, Poetry, Bundler, Cargo) to delay installation of newly published versions; and audit globally installed CLI tools for available updates, diff changes, and provide security-reviewed verdicts (SAFE/REVIEW/BLOCK) before upgrading.
| `podcast-generation` | user | AI-powered podcast script generation and audio synthesis using AWS TTS or local VibeVoice MLX on Apple Silicon. Local skill ships bundled demo voices (Alice/Frank/Carter) and normalizes voices and output to broadcast loudness standards (EBU R128 / Apple Podcasts) |
...
| `youtube` (`youtube-cli`) | user | Structured CLI for YouTube: list channel/playlist videos with filters (date, views, duration), list playlists, fetch full video metadata, download captions as VTT — all via yt-dlp library with local caching |
...
| `software-management` (`audit-software-upgrade`) | developer-tools | Audit a candidate upgrade of a GitHub-hosted app before installing (audit-and-report only, never installs): resolve current/target versions via Homebrew, the app itself, or the user; review every commit in the range for malware and critical/high vulnerabilities; audit third-party dependency changes against advisory databases (GitHub Advisory DB, OSV, ecosystem auditors) and flag dependency versions published less than 72 hours ago; decide trust the provider's prebuilt binaries vs build from source per a per-app trust policy; emit a SAFE/REVIEW/BLOCK verdict and save the full report to a timestamped `/tmp` file. Ships per-app specifications (template + cmux example) |
