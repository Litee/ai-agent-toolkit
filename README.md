...
| `ec2-vscode` (`launch-ec2-vscode`) | developer-tools | Launch or resume an EC2 instance and open VS Code Remote-SSH. Manages full session lifecycle: key pair + security group creation, SSH config, VS Code open, auto-stop on close. Teardown destroys instance, key pair, security group, and SSH config entry |
